...
def cluster_analysis_2d(
    ...
    # Get 2D clusters
    p_values = permutation.permutation_2d(
        data_a=data_a,
        data_b=data_b,
        n_perm=n_perm,
...
def clusters_from_pvals(
    ...
    if p_vals_signif.size > 1:
        p_vals_corr = timeseries.correct_pvals(
            p_vals=p_vals,
            alpha=alpha,
...
def _null_distribution_1d_onesample(
    ...
    for n in range(n_perm):
        ...
        p_values = permutation.permutation_1d_onesample(
            data_a=data_a_final,
            data_b=data_b,
            n_perm=n_perm,
            two_tailed=two_tailed,
...
def _null_distribution_1d_twosample(
    ...
    null_distr = np.empty((n_perm,))
    for n in range(n_perm):
        p_values = permutation.permutation_1d_twosample(
            data_a=data_a,
            data_b=data_b,
            n_perm=n_perm,
            two_tailed=two_tailed,
...
def cluster_analysis_1d(
    ...
    if data_b is None:
        data_b = 0
    if isinstance(data_b, (int, float)):
        f_perm = permutation.permutation_1d_onesample
        f_null_distr = _null_distribution_1d_onesample
    else:
        f_perm = permutation.permutation_1d_twosample
